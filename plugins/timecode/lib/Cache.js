...
class Cache {
   ...
   */
  store (key, data, lifetime = DEFAULT_ENTRY_LIFETIME_MS) {
    logger.debug('Caching data for key', key)
    const entry = new CacheEntry(data, lifetime)
   ...
   * @param { Function.<any> } fn A function returning a promise
   *                              resolving to some data to cache
   * @param { Number }         lifetime The lifetime of the data in milliseconds
   * @returns
   */
  cache (key, fn, lifetime) {
