...
const logger = new Logger({ name: 'Electron' })

/*
Bypass proxy for all requests to localhost
*/
...
function hardQuit (window, workspace) {
  ...
  /*
  Thses should be called after destroying the main window
  as tearing down the workspace will cause all related
  windows to close – also trying to close the main window
  if it isn't already
  */
...
async function initWorkspaceMainWindow (workspace, opts = {}) {
  ...
  workspace.saveFn = () => save(window, false)
...
async function initStatelessWindow (url, opts = {}, workspaceOpts = {}) {
  ...
  if (workspaceOpts.workspace && workspaceOpts.windowId) {
    ...
    window.webContents.on('did-finish-load', () => {
      window.webContents.executeJavaScript(`
        window.BRIDGE_TOKEN = '${token}'
        window.BRIDGE_WINDOW_ID = '${workspaceOpts.windowId}'
      `)
    })
...
/**
 * Open a project file as a new workspace in a new window
 * @param { String } filePath
 * @returns { Promise.<Void> }
 */
 ...
 */
async function save (window, allowSaveAs = true) {
