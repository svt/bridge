...
class Workspace extends EventEmitter {
  ...
  _setup () {
    ...
    */
    this.state.on('change', (state, opts) => {
      ...
      apply them to the userdefaults-state as well
      */
      if (state._userDefaults) {
        UserDefaults.apply(state._userDefaults, { workspace: this.id })
      }
    })
   ...
   * @private
   *
   * Triggered whenever the userdefaults-state
   * is changed in order to update the workspace-state
   * accordingly
   * @param { any } state
   * @param {{
   *  workspace?: String
   * }} transparent A transparent value set by the caller of .apply
   */
  _onUserDefaultsChange (state, transparent) {
    if (transparent.workspace === this.id) return

    this.state.apply({
      _userDefaults: state
    }, {
      originatedFromUserDefaults: true
    })
  }
