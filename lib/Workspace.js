  ...
  _setup () {
    ...
    */
    ...
    this.cleanupTask = setInterval(
      () => {
        if (electron.isSuspended()) {
          logger.debug('Skipping cleanup as the system is currently suspended')
          return
        }

        if (electron.lastResumed() > Date.now() - SOCKET_CLEANUP_INTERVAL_MS) {
          logger.debug('Skipping cleanup as the system recently resumed')
          return
        }
        ...
        root: {
          ...
          children: {
            1: {
              ...
              layout: {
                getting_started: { x: 0, y: 0, w: 3, h: 3 },
                state: { x: 0, y: 3, w: 3, h: 3 },
                rundown: { x: 3, y: 0, w: 6, h: 6 },
                inspector: { x: 9, y: 0, w: 3, h: 6 }
              },
              children: {
                getting_started: {
                ...
                state: {
                  component: 'bridge.plugins.state'
                },
                rundown: {
