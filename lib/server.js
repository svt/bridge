 ...
 */
const server = app.listen(HTTP_PORT, HTTP_BIND_ADDR, err => {
  if (err) {
    logger.error(err)
  } else {
    logger.info('Listening on', `${HTTP_BIND_ADDR}:${HTTP_PORT}`)
  }
})
...
app.get('/embed', async (req, res, next) => {
  ...
  if (!url) {
    return res.redirect(`/error?message=${encodeURIComponent('Invalid URL')}`)
  }

  try {
    const headers = await embed.getHeaders(url)
    const canBeEmbedded = embed.canBeEmbedded(headers)

    if (!canBeEmbedded) {
      return res.redirect(`/error?heading=${encodeURIComponent('Embed blocked')}&message=${encodeURIComponent('The requested site does not allow to be embedded')}`)
    }
  } catch (err) {
    return next(err)
  }
...
/*
Handle named urls and redirect to the
corresponding id-based url if
a match is found
*/
function handleNamedUrl (req, res, next) {
  const name = req.params.name
  const allWorkspaces = WorkspaceRegistry.getInstance().list()
  const workspace = allWorkspaces.find(workspace => {
    return workspace.state.data.url === name
  })
...
app.use('/named/:name', handleNamedUrl)
