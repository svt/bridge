...
function factory (workspace, manifest) {
  ...
  function activate () {
    function handleError (err) {
      ...
      */
      if (err.code === new WorkerError().code) {
        ...
        logger.warn('Worker exited due to error', err)
        return
      }
      throw err
    }
    ...
    try {
      ...
      logger.error('Fatal error when setting up', manifest?.name)
      handleError(err)
    }
    ...
    */
    ctx.worker.on('message', async msg => {
      if (!msg.command) {
        return
      }
      ...
      */
      try {
        const authorized = await ctx.workspace.api.auth.authorizeCommand(msg.command, msg.args, msg?.headers?.authentication)
        if (!authorized) {
