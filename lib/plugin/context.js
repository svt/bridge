    ...
    */
    ctx.worker.on('message', msg => {
      ...
      if (msg.command === 'events.triggerCommand') {
        ctx.workspace.api.commands.authorizeAndExecuteCommand({}, msg.command, ...(msg.args || []), ctx.id)
        return
      }

      ctx.workspace.api.commands.authorizeAndExecuteCommand({}, msg.command, ...(msg.args || []))
    })
  }
