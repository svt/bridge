  ...
  loadFromManifest (manifest) {
    ...
    */
    const contributionKinds = [
      ...
      { extract: this.getSettings, register: (contribution, manifest) => this._opts.workspace.api.settings.registerSetting(manifest.name, contribution) },
    ...
    for (const kind of contributionKinds) {
      ...
      for (const contribution of contributions) {
        try {
          kind.register(contribution, manifest)
        } catch (err) {
