  ...
  loadFromManifest (manifest, api) {
    ...
    */
    const contributionKinds = [
      { extract: this.getTypes, register: (...args) => api.types.registerType(...args) },
      { extract: this.getSettings, register: (...args) => api.settings.registerSetting(...args) },
      { extract: this.getShortcuts, register: (...args) => api.shortcuts.registerShortcut(...args) }
    ]
    ...
    for (const kind of contributionKinds) {
      ...
      for (const contribution of contributions) {
        try {
          ...
          logger.warn('Failed to register contribution:', err.message, err)
        }
      }
    }
  }
