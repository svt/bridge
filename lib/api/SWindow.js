// SPDX-FileCopyrightText: 2022 Sveriges Television AB
//
// SPDX-License-Identifier: MIT

/**
 * @file Implements the window API
 * @description This API is INTERNAL and may change anytime,
 *              it is not meant to be used by plugin developers
 */

const desktop = require('../desktop')
const { initStatelessWindow, MENU_TEMPLATE } = desktop

const InvalidArgumentError = require('../error/InvalidArgumentError.js')

const DIController = require('../../shared/DIController')
const DIBase = require('../../shared/DIBase')

const random = require('../security/random.js')

class SWindow extends DIBase {
  constructor (...args) {
    super(...args)
    this.#setup()
  }

  #appMenu

  #setup () {
    this.props.SCommands.registerAsyncCommand('window.openChildWindow', this.openChildWindow.bind(this))
    this.props.SCommands.registerAsyncCommand('window.openExternal', this.openExternal.bind(this))
    this.props.SCommands.registerAsyncCommand('window.getAppMenu', this.getAppMenu.bind(this))
    this.props.SCommands.registerCommand('window.setControlColors', this.setControlColors.bind(this))
    this.props.SCommands.registerCommand('window.setStayOnTop', this.setStayOnTop.bind(this))
    this.props.SCommands.registerCommand('window.toggleMaximize', this.toggleMaximize.bind(this))
    this.props.SCommands.registerCommand('window.minimize', this.minimize.bind(this))
    this.props.SCommands.registerCommand('window.close', this.close.bind(this))
  }

  #buildAppMenu (itemList) {
    const out = []

    for (const item of itemList) {
      const newItem = {
        label: item.label
      }

      if (item?.type === 'separator') {
        newItem.type = 'divider'
        out.push(newItem)
        continue
      }
      newItem.type = 'item'

      if (Array.isArray(item?.submenu)) {
        newItem.children = this.#buildAppMenu(item.submenu)
      }

      if (typeof item?.click === 'function') {
        async function onClickWrapper (windowId) {
          if (!windowId) {
            return
          }

          const window = await this.props.Workspace.props.WindowStore.getWindow(windowId)
          if (!window) {
            return false
          }
          item.click(undefined, window)
        }

        const commandName = `window.appMenu.${random.string(10)}`
        this.props.SCommands.registerCommand(commandName, onClickWrapper.bind(this))
        newItem.command = commandName
      }

      out.push(newItem)
    }

    return out
  }

  /**
   * Toggle the maximized state of the window
   * currently hosting the workspace
   * @returns { Promise.<Boolean> }
   */
  async toggleMaximize (id) {
    if (!id) {
      return
    }

    const window = await this.props.Workspace.props.WindowStore.getWindow(id)
    if (!window) {
      return false
    }

    if (window.isMaximized()) {
      window.unmaximize()
    } else {
      window.maximize()
    }
  }

  async minimize (id) {
    if (!id) {
      return
    }

    const window = await this.props.Workspace.props.WindowStore.getWindow(id)
    if (!window) {
      return false
    }

    if (window.minimizable) {
      window.minimize()
    }
  }

  async close (id) {
    if (!id) {
      return
    }

    const window = await this.props.Workspace.props.WindowStore.getWindow(id)
    if (!window) {
      return false
    }

    if (window.closable) {
      window.close()
    }
  }

  async setStayOnTop (id, newValue) {
    if (!id) {
      return
    }

    const window = await this.props.Workspace.props.WindowStore.getWindow(id)
    if (!window) {
      return false
    }

    window.setAlwaysOnTop(!!newValue)
  }

  async setControlColors (id, colors) {
    if (!id) {
      return
    }

    const window = await this.props.Workspace.props.WindowStore.getWindow(id)
    if (!window) {
      return false
    }

    if (typeof colors.symbolColor !== 'string') {
      throw new InvalidArgumentError('Invalid or missing property symbolColor')
    }

    /*
    Make sure that the function is set within Electron,
    it isn't for macOS e.t.c.
    */
    if (typeof window?.setTitleBarOverlay !== 'function') {
      return
    }

    window.setTitleBarOverlay({
      color: '#00000000',
      symbolColor: colors.symbolColor
    })
  }

  /**
   * Open a URL with an external browser
   * @param { string } url
   * @returns
   */
  openExternal (url) {
    return desktop.openExternal(url)
  }

  async openChildWindow (url) {
    const id = this.props.Workspace.props.WindowStore.getNewWindowId()
    /*
    initStatelessWindow is async on Electrobun — must be awaited so we
    don't stash an unresolved Promise in WindowStore.
    */
    const child = await initStatelessWindow(url, {
      width: 600,
      height: 400
    }, { workspace: this.props.Workspace, windowId: id })
    this.props.Workspace.props.WindowStore.addWindow(id, child)
  }

  getAppMenu () {
    if (!this.#appMenu) {
      this.#appMenu = this.#buildAppMenu(MENU_TEMPLATE)
    }
    return this.#appMenu
  }
}

DIController.main.register('SWindow', SWindow, [
  'Workspace',
  'SCommands',
  'SEvents'
])
