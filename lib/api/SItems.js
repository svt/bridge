...
class SItems extends DIBase {
  ...
  #setup () {
    ...
    this.props.SCommands.registerAsyncCommand('items.seekItem', this.seekItem.bind(this))
  ...
  playItem (item, opts = {}) {
    ...
    */
    const endDelay = Math.max(getEffectiveDuration(item), 0)
  ...
  /**
   * Seek a playing item to a new position
   * without re-emitting item.play
   *
   * Recalculates willStartPlayingAt / didStartPlayingAt
   * and reschedules items.endItem at the remaining duration
   *
   * @param { Item } item
   * @param { number } positionMs  How far into the item's duration to seek
   */
  seekItem (item, positionMs) {
    if (!item?.id) {
      throw new ApiError('Invalid item object', 'ERR_API_ITEMS_INVALID_ITEM')
    }

    const startedAt = Date.now() - positionMs
    const totalDuration = Math.max(getEffectiveDuration(item), 0)
    const remainingDuration = Math.max(totalDuration - positionMs, 0)

    this.props.SCommands.executeCommand('scheduler.abort', undefined, `end:${item.id}`)
    if (!Number.isNaN(remainingDuration) && remainingDuration > 0) {
      this.props.SCommands.executeCommand('scheduler.delay', undefined, `end:${item.id}`, remainingDuration, 'items.endItem', item)
    } else {
      this.endItem(item)
    }
    ...
    this.props.SState.applyState({
      items: {
        [item.id]: {
          state: 'playing',
          didStartPlayingAt: startedAt,
          willStartPlayingAt: startedAt
        }
      }
    })
  }
