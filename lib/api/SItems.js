// SPDX-FileCopyrightText: 2023 Sveriges Television AB
//
// SPDX-License-Identifier: MIT

const DIController = require('../../shared/DIController')
const DIBase = require('../../shared/DIBase')

const ApiError = require('../error/ApiError')
const PlayHistory = require('./PlayHistory')

const itemHelpers = require('../../shared/items')

class SItems extends DIBase {
  #playHistory = new PlayHistory()
  #stopHistory = new PlayHistory()
  #abortHistory = new PlayHistory()

  constructor (...args) {
    super(...args)
    this.#setup()
  }

  get #isLoopPreventionEnabled () {
    return this.props.SState.getState('core.internals.enableLoopPrevention')
  }

  #setup () {
    this.props.SCommands.registerAsyncCommand('items.getItem', this.getItem.bind(this))
    this.props.SCommands.registerAsyncCommand('items.endItem', this.endItem.bind(this))
    this.props.SCommands.registerAsyncCommand('items.playItem', this.playItem.bind(this))
    this.props.SCommands.registerAsyncCommand('items.stopItem', this.stopItem.bind(this))
    this.props.SCommands.registerAsyncCommand('items.abortItem', this.abortItem.bind(this))
    this.props.SCommands.registerAsyncCommand('items.deleteItems', this.deleteItems.bind(this))
    this.props.SCommands.registerAsyncCommand('items.scheduleItem', this.scheduleItem.bind(this))
  }

  #validateItemObject (item) {
    if (!item?.id) {
      throw new ApiError('Invalid item object', 'ERR_API_ITEMS_INVALID_ITEM')
    }
    return true
  }

  /**
   * Play an item
   *
   * @typedef {{
   *  id: string
   * }} Item
   *
   * @typedef {{
   *  immediate?: boolean
   * }} PlayOpts
   *
   * @param { Item } item
   * @param { PlayOpts } opts
   */
  playItem (item, opts = {}) {
    this.#validateItemObject(item)

    if (this.#isLoopPreventionEnabled && this.#playHistory.record(item.id)) {
      this.props.SMessages.createWarningMessage({
        text: `Detected play loop, Bridge prevented the item with id '${item.id}' from being repeatedly played`,
        ttl: 30000,
        dismissable: true
      })
      return
    }

    this.props.SCommands.executeCommand('scheduler.abort', undefined, `play:${item.id}`)

    /*
    Schedule a call to items.endItem if there's
    a specified duration so that we can fire the item.end event
    */
    const endDelay = Math.max(itemHelpers.getEffectiveDuration(item), 0)
    if (!Number.isNaN(endDelay) && endDelay > 0) {
      this.props.SCommands.executeCommand('scheduler.delay', undefined, `end:${item.id}`, endDelay, 'items.endItem', item)
    } else {
      this.props.SCommands.executeCommand('scheduler.abort', undefined, `end:${item.id}`)
      this.endItem(item)
    }

    this.props.SState.applyState({
      items: {
        [item.id]: {
          state: 'playing',
          didStartPlayingAt: Date.now(),
          willStartPlayingAt: Date.now()
        }
      }
    })

    this.props.SEvents.emit('item.play', item)
  }

  /**
   * Mark the end of an item
   * and emit the item.end event
   * @param { Item } item
   */
  endItem (item) {
    this.#validateItemObject(item)
    this.props.SEvents.emit('item.end', item)
  }

  /**
   * Abort a scheduled or ongoing item
   * without emitting end or stop
   * @param { Item }
   */
  abortItem (item) {
    this.#validateItemObject(item)

    if (this.#isLoopPreventionEnabled && this.#abortHistory.record(item.id)) {
      this.props.SMessages.createWarningMessage({
        text: `Detected abort loop, Bridge prevented the item with id '${item.id}' from being repeatedly aborted`,
        ttl: 30000,
        dismissable: true
      })
      return
    }

    this.props.SCommands.executeCommand('scheduler.abort', undefined, `play:${item.id}`)
    this.props.SCommands.executeCommand('scheduler.abort', undefined, `end:${item.id}`)

    this.props.SState.applyState({
      items: {
        [item.id]: {
          state: 'idle',
          willStartPlayingAt: { $delete: true },
          didStartPlayingAt: { $delete: true }
        }
      }
    })

    this.props.SEvents.emit('item.abort', item)
  }

  /**
   * Schedule an item to be
   * played after a certain delay
   *
   * @param { Item } itemId
   * @param { number } delay
   */
  scheduleItem (item, delay) {
    this.#validateItemObject(item)

    if (delay == null) {
      this.playItem(item)
      return
    }

    this.props.SState.applyState({
      items: {
        [item.id]: {
          state: 'scheduled',
          wasScheduledAt: Date.now(),
          willStartPlayingAt: Date.now() + delay
        }
      }
    })

    this.props.SCommands.executeCommand('scheduler.delay', undefined, `play:${item.id}`, delay, 'items.playItem', item)
    this.props.SEvents.emit('item.schedule', item)
  }

  /**
   * Stop an item by its id
   * @param { Item } item
   */
  stopItem (item) {
    this.#validateItemObject(item)

    if (this.#isLoopPreventionEnabled && this.#stopHistory.record(item.id)) {
      this.props.SMessages.createWarningMessage({
        text: `Detected stop loop, Bridge prevented the item with id '${item.id}' from being repeatedly stopped`,
        ttl: 30000,
        dismissable: true
      })
      return
    }

    this.props.SCommands.executeCommand('scheduler.abort', undefined, `play:${item.id}`)
    this.props.SCommands.executeCommand('scheduler.abort', undefined, `end:${item.id}`)

    this.props.SState.applyState({
      items: {
        [item.id]: {
          state: 'stopped',
          willStartPlayingAt: { $delete: true },
          didStartPlayingAt: { $delete: true }
        }
      }
    })

    this.props.SEvents.emit('item.stop', item)
  }

  /**
   * Get a single item from the state
   * @param { String } id The id of the item to retrieve
   * @returns { any | undefined }
   */
  getItem (id) {
    return this.props.Workspace.state.data?.items?.[id]
  }

  /**
   * Remove a set of items
   * from the item-state,
   *
   * will trigger the
   * items.delete event
   *
   * Handling of parent-child relations
   * are done within the rundown-plugin
   *
   * @param { String[] } ids An array of ids to delete
   */
  async deleteItems (ids) {
    if (ids.length === 0) {
      return
    }

    /*
    Fetch all items and filter out
    any non-existing items before
    operating on them
    */
    const items = ids
      .map(id => this.getItem(id))
      .filter(item => item)

    const patch = {}
    for (const item of items) {
      patch[item.id] = { $delete: true }
      this.#playHistory.delete(item.id)
      this.#stopHistory.delete(item.id)
    }
    this.props.SState.applyState({
      items: patch
    })

    this.props.SEvents.emit('items.delete', items)
  }
}

DIController.main.register('SItems', SItems, [
  'Workspace',
  'SCommands',
  'SMessages',
  'SEvents',
  'SState'
])
